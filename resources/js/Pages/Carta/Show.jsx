                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden mb-10 transition-colors duration-300">
                    {carta.imagem_path && (
                        <div className="w-full h-64 md:h-96 bg-gray-200 dark:bg-gray-700 flex items-center justify-center bg-black/5 dark:bg-black/20">
                             <img src={`/storage/${carta.imagem_path}`} className="w-full h-full object-contain" alt="Capa" />
                        </div>
                    )}
                    
                    <div className="p-8 md:p-12">
                        <div className="flex flex-wrap items-center gap-4 mb-6">
                            <span className="flex items-center gap-2 text-red-700 dark:text-red-300 font-bold bg-red-50 dark:bg-red-900/20 px-4 py-1 rounded-full">
                                <FaCalendarAlt /> {formatarData(carta.created_at)}
                            </span>
                            <span className="flex items-center gap-2 text-gray-600 dark:text-gray-300 font-bold bg-gray-100 dark:bg-gray-700 px-4 py-1 rounded-full">
                                <FaUserTie /> {carta.autor}
                            </span>
                        </div>

                        <h1 className="text-3xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-6 leading-tight">{carta.titulo}</h1>
                        
                        <p className="text-xl text-red-900 dark:text-red-300 font-medium mb-8 italic border-l-4 border-red-400 dark:border-red-600 pl-4 leading-relaxed">
                            {carta.chamativo}
                        </p>
                ...
                {carta.fotos && carta.fotos.length > 0 && (
                    <div className="mt-12">
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-6 flex items-center gap-2 border-b dark:border-gray-700 pb-2">
                            <FaImages className="text-red-700 dark:text-red-400" /> Galeria de Imagens
                        </h2>
