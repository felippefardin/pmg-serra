...
export default function Edit({ noticia }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT', // Necessário para enviar arquivos no update do Laravel
        titulo: noticia.titulo || '',
        chamativo: noticia.chamativo || '', // Novo campo
        conteudo: noticia.conteudo || '',
        capa: null,   // Alterado de 'imagem' para 'capa' para bater com o Controller
        galeria: [],  // Novo campo
    });
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-dashed border-gray-300 dark:border-gray-600">
                                <label className="block text-gray-700 dark:text-gray-300 font-bold mb-2 flex items-center gap-2">
                                    <FaImage className="text-indigo-500" /> Nova Foto de Capa (Opcional)
                                </label>
                            ...
                            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl border border-dashed border-gray-300 dark:border-gray-600">
                                <label className="block text-gray-700 dark:text-gray-300 font-bold mb-2 flex items-center gap-2">
                                    <FaImages className="text-indigo-500" /> Adicionar à Galeria
                                </label>
                                <input 
                                    type="file" 
                                    multiple 
                                ...
                                />
                                <p className="text-xs text-gray-400 mt-2">Segure Ctrl (ou Cmd) para selecionar várias.</p>
                            </div>
                        </div>
                    </div>
