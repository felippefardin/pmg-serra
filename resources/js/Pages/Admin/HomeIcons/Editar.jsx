...
import { FaTrash, FaPlus, FaImages, FaUndo } from 'react-icons/fa';
...
export default function EditarIcone({ auth, icon }) {
    ...
    // NOTA: Usamos 'post' em vez de 'put' para permitir o upload de arquivos, 
    // enviando _method: 'PUT' para o Laravel entender que é uma atualização.
    const { data, setData, post, processing, errors } = useForm({
        ...
        imagens: [], // Para novas imagens
        imagens_removidas: [] // Para armazenar caminhos das imagens a serem deletadas
    });
    ...
    // Funções para gerenciar imagens existentes
    const markImageForRemoval = (path) => {
        if (!data.imagens_removidas.includes(path)) {
            setData('imagens_removidas', [...data.imagens_removidas, path]);
        }
    };

    const undoImageRemoval = (path) => {
        setData('imagens_removidas', data.imagens_removidas.filter(p => p !== path));
    };
                        ...
                        <form onSubmit={submit} className="space-y-6" encType="multipart/form-data">
                            ...
                            <div className="border-t pt-4 dark:border-gray-700">
                                            ...
                                            return (
                                                <div key={idx} className={`relative group border rounded-lg overflow-hidden ${isRemoved ? 'opacity-50 grayscale' : ''}`}>
                                                    ...
                                                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                                                        {isRemoved ? (
                                                            <button type="button" onClick={() => undoImageRemoval(img)} className="bg-green-600 text-white p-2 rounded-full text-xs flex items-center gap-1 hover:bg-green-700 transition">
                                                                <FaUndo /> Restaurar
                                                            </button>
                                                        ) : (
                                                            <button type="button" onClick={() => markImageForRemoval(img)} className="bg-red-600 text-white p-2 rounded-full text-xs flex items-center gap-1 hover:bg-red-700 transition">
                                                                <FaTrash /> Remover
                                                            </button>
                                                        )}
                                                    </div>
                                                    {isRemoved && <div className="absolute top-0 right-0 bg-red-600 text-white text-xs px-2 py-1">Excluída</div>}
                                                </div>
                                            );
                                        })}
                                    </div>
                                ...
                                <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
                                    <label className="block font-bold text-gray-700 dark:text-gray-300 mb-2">Adicionar Novas Imagens</label>
                                    <input 
                                        type="file" 
                                        multiple 
                                        accept="image/*"
